...
class Quees
{
    ...
    private static function existe_titulo_duplicado($titulo, $idTecnologia, $excludeId = null)
    {
        ...
        $sql = "SELECT 1 FROM quees WHERE LOWER(TRIM(titulo)) = LOWER(TRIM(?)) AND id_tecnologia = ?";
    ...
    public function guardar()
    {
        ...
        $sql = "INSERT INTO quees (id_tecnologia, titulo, contenido, imagen, link_referencia, activo, orden) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $objConn = new Conexion();
        $stmt = $objConn->enlace->prepare($sql);
        if (!$stmt) {
            return false;
    ...
    public function actualizar($nro = 0)
    {
        ...
        $sql = "UPDATE quees SET id_tecnologia = ?, titulo = ?, contenido = ?, imagen = ?, link_referencia = ?, activo = ?, orden = ? WHERE id_quees = ?";
        $objConn = new Conexion();
        $stmt = $objConn->enlace->prepare($sql);
        if (!$stmt) {
            return false;
    ...
    {
        $sql = "DELETE FROM quees WHERE id_quees = ?";
        $objConn = new Conexion();
        $stmt = $objConn->enlace->prepare($sql);
        if (!$stmt) {
            return false;
    ...
    {
        if ($nro != 0) {
            $sql = "SELECT * FROM quees WHERE id_quees = ?";
            $objConn = new Conexion();
            $stmt = $objConn->enlace->prepare($sql);
            if (!$stmt) {
                return [];
    ...
    {
        $sql = "SELECT * FROM quees WHERE titulo LIKE ? OR contenido LIKE ? OR link_referencia LIKE ? OR id_quees = ?";
        $objConn = new Conexion();
        $stmt = $objConn->enlace->prepare($sql);
        if (!$stmt) {
            return [];
    ...
    {
        $sql = "SELECT * FROM quees WHERE id_tecnologia = ? AND activo = 1 ORDER BY orden";
        $objConn = new Conexion();
        $stmt = $objConn->enlace->prepare($sql);
        if (!$stmt) {
            return [];
    ...
    {
        $sql = "SELECT id_tecnologia, COUNT(id_quees) AS total FROM quees WHERE activo = 1 GROUP BY id_tecnologia";
        $objConn = new Conexion();
        $stmt = $objConn->enlace->prepare($sql);
        if (!$stmt) {
            return [];
    ...
    {
        $sql = "SELECT * FROM quees WHERE titulo LIKE ? OR id_tecnologia = ? ORDER BY orden";
        $objConn = new Conexion();
        $stmt = $objConn->enlace->prepare($sql);
        if (!$stmt) {
            return [];
