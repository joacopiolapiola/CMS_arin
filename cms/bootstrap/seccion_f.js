     $(document).ready(function(){
        ...
        window.setTimeout(function () { $("#btn_b_seccion").trigger("click"); }, 0);
          ...
          $("#btn_b_seccion").click(function(e){
              ...
              consulta = $("#txt_b_seccion").val();
			  //alert (consulta);
              //hace la búsqueda                                                                                  
              $.ajax({
                    type: "POST",
                    url: "secciones_busqueda_cms.php",
                    data: "b="+consulta,
                    dataType: "html",
                    beforeSend: function(){
                    //imagen de carga
        ...
        $("#txt_b_seccion").keyup(function(e){
		  if (e.which != 13
                        )return;           
              //obtenemos el texto introducido en el campo de búsqueda
              consulta = $("#txt_b_seccion").val();
...
function editar(id) {
   //alert('EDITAR '+id);
    ...
    $.ajax({
        type: "POST",
        url: "seccion_edit.php",
        data: {operacion: 'edicion', id_seccion: id}
    }).done(function (html) {
        $('#Contenido').html(html);
    });
}
...
function borrar(id) {
   //alert('BORRAR '+id);
    ...
    $.ajax({
        type: "POST",
        url: "seccion_edit.php",
        data: {operacion: 'baja', id_seccion: id}
    }).done(function (html) {
        $('#Contenido').html(html);
    });
}
