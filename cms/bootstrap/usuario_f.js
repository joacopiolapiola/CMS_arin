$(document).ready(function(){
    ...
    window.setTimeout(function () { $("#btn_b_usuario").trigger("click"); }, 0);
      ...
      $("#btn_b_usuario").click(function(e){
          ...
          consulta = $("#txt_b_usuario").val();
          //alert (consulta);
          //hace la búsqueda                                                                                  
          $.ajax({
                type: "POST",
                url: "usuarios_busqueda_cms.php",
                data: "b="+consulta,
                dataType: "html",
                beforeSend: function(){
                //imagen de carga
    ...
    $("#txt_b_usuario").keyup(function(e){
      if (e.which != 13
                    )return;           
          //obtenemos el texto introducido en el campo de búsqueda
          consulta = $("#txt_b_usuario").val();
...
$.ajax({
    ...
    data: {operacion: 'edicion', id_usuario: id}
}).done(function (html) {
    $('#Contenido').html(html);
});
}
...
$.ajax({
    type: "POST",
    url: "usuario_edit.php",
    data: {operacion: 'baja', id_usuario: id}
}).done(function (html) {
    $('#Contenido').html(html);
});
}
