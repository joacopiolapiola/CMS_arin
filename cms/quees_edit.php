...
if (!empty($_POST)) {
   ...
   $operacion = isset($_POST['operacion']) ? $_POST['operacion'] : 'actualizar' ;
    ...
    $orden= $A['orden'];
		...
		$accion = $_SERVER['PHP_SELF'].'?operacion=actualizar&id_quees='. $id;
		$btn_txt='Actualizar';
		$leyenda='Modificar Que es ';
		
	}
	
  if ($operacion == 'actualizar' && isset($_GET['id_quees'])){
    //echo '2-actualizar';
    $quees->titulo=$_POST['txtTitulo'];
    $quees->id_tecnologia=$_POST['txtId_tecnologia'];
    $quees->contenido=$_POST['txtContenido'];
    $quees->imagen=$_POST['txtImg'];
    $quees->orden=$_POST['txtOrden'];
    $quees->link_referencia=$_POST['txtEnlace'];
    $quees->activo=$_POST['txtActivo'];
 
    $quees->actualizar($_GET['id_quees']);
...
$accion=$_SERVER['PHP_SELF'].'?operacion=borrar&id_quees='. $id;
$btn_txt='Borrar';
$leyenda='Eliminar';
}
...
if (!empty($_GET)) {
  ...
  if ($operacion == 'borrar' && isset($_GET['id_quees'])){
    //echo '2-actualizar';
    
  
  $quees->borrar($_GET['id_quees']);
  header("Location: ".$_SERVER['HTTP_REFERER']);
  }
